...
class DataManager:
        ...
        for i, file_name in enumerate(file_names):
            ...
            if (
                ...
                if not file_name.endswith(".txt"):
                    ...
                    self.sensor_list.extend(file_name)
    ...
    def unified_data(self, data_dict, merge_time=5, save=False):
        ...
        # combine sensor data
        for df in data_dict.values():
            df["timestamp"] = pd.to_timedelta(df["timestamp"], "ms") / np.timedelta64(
                "1", "ms"
            )
            self.data = pd.merge_asof(
                ...
                df,
                on="timestamp",
                tolerance=pd.to_timedelta(1, "ms") / np.timedelta64("1", "ms"),
            )
        if save:
    ...
    def plot_feature(self, data, feature_name=None, activity=None):
        ...
        if not activity:
            ...
            if not activity_on.empty:
                activity_on.plot(
                    kind="scatter", x="timestamp", y=feature_name, ax=ax, color="r"
                )
            else:
    ...
    def __get_act_intervals(self, data: pd.DataFrame, act: str):
        ...
        segments.append(
            [desired_act[c[-1] + 1, 0], desired_act[len(desired_act) - 1, 0]]
        )
